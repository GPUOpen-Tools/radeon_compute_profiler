...
#endif
        MessageBox(NULL, errorMessageConverted.c_str(), _T("rcprof"),
...
std::string FileUtils::GetTempActivityLoggerParamsFile()
{
    std::string strTmpPath;
#ifdef _WIN32
    strTmpPath = OSUtils::Instance()->GetEnvVar("TEMP");
    strTmpPath.append("\\rcpdata" PERFMARKER_EXT);
#else //_LINUX || LINUX
    strTmpPath = OSUtils::Instance()->GetEnvVar("HOME");
    strTmpPath.append("/.rcpdata" PERFMARKER_EXT);
#endif
...
void FileUtils::PassParametersByFile(Parameters params)
{
    ...
    // if this implementation changes, update the ActivityLogger too
    fout.open(GetTempActivityLoggerParamsFile().c_str());
    fout << "TimeOut=" << (params.m_bTimeOutBasedOutput ? "True" : "False") << endl;

    std::string tempPath;
    std::string outputFile;

    if (params.m_strOutputFile.empty())
    {
        tempPath = FileUtils::GetDefaultOutputPath();
        outputFile = FileUtils::GetDefaultPerfMarkerOutputFile();
    }
    else
    {
        tempPath = FileUtils::GetTempFragFilePath();
        outputFile = FileUtils::GetBaseFileName(params.m_strOutputFile) + PERFMARKER_EXT;
    }

    fout << "PerfMarkerTempFileBaseName=" << tempPath << endl;
    fout << "PerfMarkerOutputFileName=" << outputFile << endl;
...
bool FileUtils::GetParametersFromFile(Parameters& params)
{
        ...
        try
        {
            while (!fin.eof())
            {
                ...
                else if (opStr.find("OutputFile") != std::string::npos)
                {
                    ...
                    // this, you can modify the default .rcpdata/tmp.rcpdata data file (the "OutputFile=" line) that is used
                    // when profiling using CL_AGENT.  You can also use this by manually including the ___PID___ string in
                    // the argument passed to the -o (--outputfile) switch in rcprof.
    ...
    if (bSave_default)
    {
        ...
        // Write config out
        // It's helpful when user use agent without rcprof,
...
void FileUtils::DeleteTmpFile()
{
    ...
    remove(GetTempActivityLoggerParamsFile().c_str());
}
...
std::string FileUtils::GetDefaultOutputPath()
{
    ...
    strcat_s(szDefaultOutputPath, SP_MAX_PATH, "\\RCP\\");
