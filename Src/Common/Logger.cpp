...
void Log(enum LogType type, const char* fmt, ...)
{
    ...
    if (truncated == false)
    {
        // Prepend "rcprof: " for windows OutputDebugString() messages
        nSize = SP_snprintf(&fullString[nLen], SP_LOG_MAX_LENGTH - nLen, "rcprof: ");
...
void LogW(enum LogType type, const wchar_t* fmt, ...)
{
    ...
    {
        // Prepend "rcprof: " for windows OutputDebugString() messages
        nSize = _snwprintf_s(&fullString[nLen], SP_LOG_MAX_LENGTH - nLen, _TRUNCATE, L"rcprof: ");
