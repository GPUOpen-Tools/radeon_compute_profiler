...
/// between rcprof and the servers.  The file is stored in %TEMP% on Windows
/// and in the user's home directory on Linux.  The file is named tmp.rcpdata on
...
/// and in the user's home directory on Linux.  The file is named rcpdata.amdtperfmarker on
/// Windows and .rcpdata.amdtperfmarker on Linux
/// \return fully-qualified file name of temp file
std::string GetTempActivityLoggerParamsFile();
...
/// Pass parameters between rcprof and servers through the temp file
...
/// Get default output path. For windows, it's "RCP"
