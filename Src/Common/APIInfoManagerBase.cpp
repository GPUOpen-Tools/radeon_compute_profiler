...
void APIInfoManagerBase::WriteTimestampToStream(std::ostream& sout)
{
    ...
    sout << ATP_SECTION_HEADER_START_END << m_strTraceModuleName << ATP_TIMESSTAMP_OUTPUT ATP_SECTION_HEADER_START_END << endl;
...
void APIInfoManagerBase::WriteAPITraceDataToStream(std::ostream& sout)
{
    ...
    TraceInfoMap& activeMap = m_TraceInfoMap[ 0 ];
    sout << ATP_SECTION_HEADER_START_END << m_strTraceModuleName << ATP_API_TRACE_OUTPUT ATP_SECTION_HEADER_START_END << endl;
...
void APIInfoManagerBase::WriteStackTraceDataToStream(std::ostream& sout)
{
    ...
    sout << ATP_SECTION_HEADER_START_END << m_strTraceModuleName << ATP_STACK_TRACE_OUTPUT ATP_SECTION_HEADER_START_END << endl;

    if (GlobalSettings::GetInstance()->m_params.m_bCompatibilityMode)
    {
        if (!GlobalSettings::GetInstance()->m_params.m_bTestMode)
        {
            sout << FILE_HEADER_PROFILER_VERSION << EQUAL_SIGN_STR << RCP_MAJOR_VERSION << "." << RCP_MINOR_VERSION << "." << RCP_BUILD_NUMBER << endl;
        }
        else
        {
            sout << FILE_HEADER_PROFILER_VERSION << EQUAL_SIGN_STR << 0 << "." << 0 << "." << 0 << endl;
        }
    }
